...
if (mysqli_num_rows($query) > 0) :
    ...
    while ($getdata = mysqli_fetch_assoc($query)) : ?>
        <tr id="<?= $getdata['id_calon_karyawan'] ?>">
            ...
            <td>
                <a href="?page=penilaian-rekrutmen&action=nilai&calon_karyawan=<?= $getdata['id_calon_karyawan'] ?>&penerimaan=<?= $id_recruitment ?>" class="btn btn-sm btn-primary <?= $getdata['status'] == 2 || $getdata['status'] == 3 ? 'disabled' : '' ?>">Nilai</a>
            </td>
        </tr>
    <?php endwhile;
else :
