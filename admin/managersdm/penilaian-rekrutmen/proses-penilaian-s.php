...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($getdata['status'] == 0) {
        ...
        mysqli_query($koneksi, "INSERT INTO `nilai_penilaian_rekrutmen`(`id_rekrutmen`, `id_calon_karyawan`, `nilai`, `status`) VALUES ('$id_recruitment', '$id_calon_karyawan', '$subnilai', 'nilaisub')");
        mysqli_query($koneksi, "INSERT INTO `nilai_penilaian_rekrutmen`(`id_rekrutmen`, `id_calon_karyawan`, `nilai`, `status`) VALUES ('$id_recruitment', '$id_calon_karyawan', '$nilaipsikotes', 'nilaisub')");

        $nilai = [0,0,0,0,0];

        mysqli_query($koneksi, "UPDATE `penilaian_rekrutmen` SET `status` = '1' WHERE `id_rekrutmen` = '$id_recruitment' AND `id_calon_karyawan` = '$id_calon_karyawan'");
