                    ...
                    <tbody>
                        ...
                        while ($getdata = mysqli_fetch_assoc($query)) : ?>
                            <tr id="<?= $getdata['id_fpkb'] ?>">
                                ...
                                <td>
                                    <?php
                                    if ($getdata['status_fpkb'] > 3 & $getdata['status_fpkb'] < 6) {
                                        echo 'Manager SDM, Direktur Suport';
                                ...
                                <td>
                                    <a href="?page=fpkb&action=penyerahan&fpkb=<?= $getdata['id_fpkb'] ?>&status=1" class="btn btn-block btn-success btn-sm mt-1 penyerahankesupport <?php if ($getdata['status_fpkb'] > 3) echo 'disabled' ?>" title="Serahkan Ke Direktur Support"><i class="fas fa-check"></i> Setujui</a>
                                    <a href="?page=fpkb&action=penyerahan&fpkb=<?= $getdata['id_fpkb'] ?>&status=0" class="btn btn-block btn-danger btn-sm mt-1 tolakfpkb <?php if ($getdata['status_fpkb'] > 3) echo 'disabled' ?>" title="FPKB Ditolak"><i class="fas fa-times"></i> Tolak</a>
